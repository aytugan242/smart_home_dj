## Запуск проекта Django с использованием Docker в командной строке:
1. Подготовили проект: обновили settings.py (база данных sqlite3), добавили .env (несколько переменных), requirements.txt (установили доп плагины и обновили файл), написали Dockerfile
2. В коммандной строке перешли в каталог с проектом Django, где находятся файлы Dockerfile и .env.
3. Собрали образ Docker, запустив команду: docker build -t my_django_project .
4. Запустили контейнер Docker, используя команду: docker run --env-file .env -p 8000:8000 my_django_project
5. Проверили проект Django по адресу http://localhost:8000/ - Все корректно открывается и работает
